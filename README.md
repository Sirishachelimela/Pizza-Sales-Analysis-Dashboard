🍕 Pizza Sales Analysis Dashboard
---
📌 Project Overview
---

This project analyzes pizza sales data to uncover key insights related to revenue, customer behavior, and product performance using an interactive dashboard.

The analysis helps understand sales trends, customer preferences, and operational efficiency to support data-driven decision-making.
<img width="600" height="300" alt="image" src="https://github.com/user-attachments/assets/e69ab0d7-2faa-4c7e-884a-5ed7bd468b28" />


🎯 Business Objectives
---

Analyze total sales and order performance

Identify peak sales days and hours

Understand customer preferences by pizza category and size

Identify best and worst performing pizzas

Provide actionable business insights

📂 Dataset Overview
---

The dataset includes:

Order date and time

Number of orders

Total pizzas sold

Revenue and average order value

Pizza category and size

📊 Key Metrics
---

Total Revenue: $817,860

Total Orders: 21,350

Total Pizzas Sold: 49,574

Average Order Value: $38.30

Average Pizzas per Order: 2.32

⏰ Sales Trends
📅 Daily Trend

Highest number of orders occur on Fridays and Saturdays

Weekend sales outperform weekdays

Consistent demand throughout the week

🕒 Hourly Trend

Peak ordering time: 12 PM – 1 PM

Second peak: 5 PM – 8 PM

Lowest activity during late-night hours

🍕 Category & Size Analysis
---
📦 Sales by Category

Classic pizzas contribute the highest share of sales

Chicken, Veggie, and Supreme categories follow closely

📐 Sales by Size

Large pizzas generate the highest revenue

Medium and Small sizes are popular

XL and XXL sizes contribute minimal revenue

🏆 Top & Bottom Performing Pizzas
🔝 Top Selling Pizzas

Classic Deluxe Pizza

Barbecue Chicken Pizza

Hawaiian Pizza

Pepperoni Pizza

Thai Chicken Pizza

🔻 Lowest Selling Pizzas

The Brie Carre Pizza

Spinach Supreme Pizza

Calabrese Pizza

📈 Key Insights
---

Sales peak during lunch and dinner hours

Few pizza types drive most of the revenue

Large-size pizzas are preferred by customers

Some pizzas underperform and may need repositioning

💡 Business Recommendations
---

Increase staffing during peak hours

Promote top-selling pizzas through combo offers

Re-evaluate low-performing pizzas

Optimize inventory based on demand trends

📌 Conclusion
---
This project provides a comprehensive overview of pizza sales performance, helping businesses optimize operations, improve customer satisfaction, and increase revenue through data-driven insights.
